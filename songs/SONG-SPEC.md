...
| 4 | **One gesture** | A single repeatable identity element: a melodic hook, a rhythmic stab, a vocal ad-lib, a chord you let sit. Templates F, G, and H all converged on this independently this week: Kokomo's lingering minor chord, the four-chord loop played twice as fast, the stab that repeats across three genre-hopping sections. Without a gesture a sketch is generic, technically complete and forgettable. |

These four map directly to `key`, `tempo`, `chords` in the content schema,
plus a new optional `gesture` field (added alongside this doc). They are
...
- Melody (the full one, not just the gesture)
- Lyrics
...
This is deliberately a gate, not a wishlist. `gesture` is the field most
