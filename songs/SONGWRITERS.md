# Reverse engineer, then forward engineer

The study curriculum. Take the people who demonstrably did it best, pull their
songs apart down to the parts, log the parts as data, then recombine the parts
into work that is yours.

This is the same loop as `ESSAYS-1000.md`, applied to music: read the source,
commit the spec, fork it, ship it.

## Two rankings, because "best" has two meanings

### By numbers · Billboard Hot 100 number-one singles as a writer

The hardest available measure of songwriting output that connected.

| # | Songwriter | No. 1s | Wrote for |
|---|---|---|---|
| 1 | **Paul McCartney** | 32 | The Beatles |
| 2 | **Max Martin** | 30 | Britney, Katy Perry, Taylor Swift, The Weeknd, Ariana Grande |
| 3 | **John Lennon** | 26 | The Beatles |
| 4 | Mariah Carey | 18 | herself |
| 4 | Dr. Luke | 18 | Katy Perry, Kesha |
| 6 | Barry Gibb | 16 | Bee Gees, Andy Gibb |

Source: Wikipedia, *List of Billboard Hot 100 chart achievements and
milestones*.

### By critics · Rolling Stone's 100 Greatest Songwriters (2015, staff vote)

| # | Songwriter |
|---|---|
| 1 | Bob Dylan |
| 2 | Paul McCartney |
| 3 | John Lennon |
| 4 | Chuck Berry |
| 5 | Smokey Robinson |
| 6 | Mick Jagger / Keith Richards |
| 7 | Carole King / Gerry Goffin |
| 8 | Paul Simon |
| 9 | Joni Mitchell |
| 10 | Stevie Wonder |

### By production · Greatest producers, cross-source consensus

No single authoritative chart exists for producers the way Billboard exists
for writers, so this is a synthesis across several ranked lists (Gear4music,
Smooth Radio, Yardbarker), not one source.

| Producer | Known for |
|---|---|
| **George Martin** | "The Fifth Beatle" — arrangement and orchestration as a writing tool, not just engineering |
| **Phil Spector** | The Wall of Sound — stacking so many players the arrangement becomes the instrument |
| **Sam Phillips** | Sun Records — didn't chase a sound, let Elvis/Cash/Perkins each keep their own |
| **Quincy Jones** | Range across jazz, pop, and film scoring in one career |
| **Dr. Dre** | Defined West Coast rap's sound, then built a label (Aftermath) as the delivery system |
| **Brian Eno** | Ambient and generative systems — treats the studio itself as an instrument |
| **Max Martin** | Already your #2 on the songwriters table, also the template for the modern pop-factory producer |

## The overlap is the whole point

**McCartney and Lennon are top 3 on BOTH lists.** Nobody else is. Critics and
the charts agree on exactly two people, and **The Beatles are already your #2
artist.**

So the curriculum is not a guess. Start where the evidence and your own taste
already point.

The second name to sit with is **Max Martin**, 30 number ones and almost no
public profile. He is the counter-argument to genius: a craftsman who reverse
engineered pop into rules he could run at volume, which is precisely the
programmer's approach you asked for. He is also, indirectly, in your top 18
(the machinery behind that whole modern-pop lane).

## Three workflows worth stealing, not just admiring

Three completely different answers to "how do you actually work," verified
against interviews and gear write-ups, not folklore.

**Avicii** worked melody-first: chords or a topline before anything else,
then built the track around it, leaning on loops and presets rather than
deep sound-design tweaking. Practical, fast, unglamorous. The lesson: the
melody decision comes before the production decision, not after.

**Aphex Twin** runs an almost entirely hardware studio — analogue sequencers
(an MC4, a Cirklon, an old Korg), obscure synths (Yamaha DX7, Casio FZ-20M),
and reportedly reconfigures the whole setup between projects. The lesson:
technical limitation is a feature, not a blocker. A smaller toolkit forces
different decisions than an infinite plugin folder does.

His actual sound-manipulation techniques, verified via DJ Mag/Splice/
Wikipedia (2026-08-14, not vibes): **break-chopping** (a drum break sliced
and reassembled via reverse/gate/stretch/pitch/reorder/stutter, past
recognition), **LFO-mapped sample-bank cycling** (~15 kick/snare/hi-hat
variants each loaded into separate sampler slots, an LFO mapped to "which
sample plays," cycling synced to a master rate knob instead of manually
programmed hit-by-hit), **layering multiple drum machines** (808 + 909
rather than one), and **iterative re-sampling** (re-sampling his own
results and processing them again, not working toward a fixed structure).
Feeds `arrange-song` directly for anything wanting non-standard drum
programming, and `create-song-hook` for the LFO-cycling idea specifically
(a hook that's a *behavior*, not a fixed phrase).

**The Beatles / Lennon-McCartney**, per Songmeaning.io, Far Out Magazine,
and Abbey Road's own account (2026-08-14): the songwriting split was real
and complementary, not a myth — McCartney usually supplied melodic
structure and polished, complete-sounding demos, Lennon supplied raw
ideas, lyrical bite, and provocative images, and each routinely rewrote
the other's lines rather than staying in a lane. Two incomplete songs were
sometimes literally spliced into one finished one. Studio-side, three
named, reproducible techniques: **Artificial Double Tracking** (invented
at Abbey Road in 1966 specifically because Lennon hated re-singing a part
twice — a tape signal routed through a second machine with its speed
nudged by an oscillator, faking a doubled vocal with one take, used on
nearly every *Revolver* track), **backwards recording** (reversed vocals
on "Rain," the first pop song to do it), and **tape loops / musique
concrète** (the sped-up loops on "Tomorrow Never Knows"). The lesson two
lessons, not one: a real co-writer who pushes back beats a solo pass, and
a genuine studio limitation (Lennon's impatience) produced a real
technique (ADT) still in use 60 years later. Feeds `arrange-song` for ADT
and tape-manipulation ideas, and is the concrete case for why
`write-philosophical-lyrics`/`create-song-hook` sessions benefit from a
second real voice pushing back on a draft, not just one writer alone.

**Rick Rubin** doesn't use a DAW at all, records straight to tape, and by
his own account has no technical training in music. He calls his role
"reducer" rather than producer, strips arrangements down instead of adding
to them, and asks artists to meditate before working with him. The lesson:
taste and reduction are a real skill, separable from technical skill
entirely.

Three different answers, same shape: each one picked a real constraint
(melody-first, hardware-only, no-DAW) and let it do the deciding instead of
staying infinitely flexible.

## What K-pop's actual production techniques are (not the sound, the moves)

From a real breakdown of 2024-2025 K-pop production (Magnetic by illit,
Crazy by The Seraphim, Supernova by aespa, She by Baby Monster, Hea by
IVE), structure only, no lyrics:

- **The demo pipeline is real and brutal.** A label can receive thousands
  of demos for one artist's "lead" (the creative brief). Songwriters keep
  the rights to a rejected demo and can resubmit it for a different artist
  later — a demo that doesn't land isn't dead, it's queued.
- **The offbeat bass-and-hihat move.** Placing bass and hi-hats on the
  offbeat against a straight kick/snare is what creates that "dancy house"
  bounce in songs like Magnetic and Crazy. One rhythmic choice, reused
  across multiple hits.
- **Vocal chopping as an arrangement decision, not an effect.** Producers
  record one full vocal take, then chop it into fragments and place them
  rhythmically, often layered with a doubler and reverse reverb for impact.
  The vocal becomes a percussion element, not just melody.
- **The "vocal as punctuation" trick.** Some songs (Fast Forward by Chungha,
  Supernova by aespa) leave the beat mostly open and let a short vocal
  phrase, often the title itself, punctuate the ends of bars instead of
  carrying the whole line.
- **Change the chords under the same topline for the final chorus.** Hea
  by IVE keeps the same melody in its last chorus but changes the harmony
  underneath it, making an already-heard hook feel bigger instead of
  repetitive. Directly usable: your existing melody, a new chord bed.
- **Arrangement switch-ups instead of a straight verse-chorus loop.** aespa's
  catalog and Baby Monster's She both change instrumentation or introduce
  an entirely new section (in She's case, a triplet-rhythm section instead
  of returning to the chorus) rather than repeating the same loop to the
  end.
- **Global crossover is now normal, not exceptional.** Non-Korean writers
  and producers (Bruno Mars on Apt., collaborators from outside Korea on
  multiple 2024-2025 releases) are increasingly credited on major K-pop
  releases. The genre boundary is porous on the production side even when
  the artist and language aren't.

## The people behind the two most-discussed K-pop identities right now

**250** (real name Lee Ho Hyoung, Seoul-based) is the main producer/writer
behind NewJeans' "Hype Boy," "Attention," "Ditto," and "ETA" — the Y2K,
UK-garage-and-jersey-club-leaning sound. Long career before NewJeans
(worked with BTS, BoA, f(x)), which matters: the "overnight sound" was
built on a decade of unrelated production work first.

**Min Hee-jin** isn't a songwriter or producer in the hands-on sense — she's
the creative director (former SM Entertainment art director, then ADOR
CEO) who set NewJeans' entire visual and musical grammar: Y2K nostalgia,
naturalism over K-pop's usual elaborate staging, members presented as
equals rather than a ranked hierarchy. Worth studying as the other half of
the K-pop demo pipeline above — 250 makes the track, Min Hee-jin is the
kind of A&R/creative-direction role that decides which of the thousands of
submitted demos actually becomes "the one," and shapes everything the demo
gets wrapped in afterward. Two different, both necessary, skills.

## Right now, for calibration (2026 charts, not historical)

What's actually charting matters as a sanity check against the historical
studies above — "great" and "current" are different questions.

**Billboard Hot 100, 2026 (as of the week of June 6, 2026):** Drake passed
Michael Jackson for most #1s by a male solo artist (his "Janice STFU" was
also the first rap #1 since Kendrick Lamar & SZA's "Luther"); other 2026
top-10 entries include Bruno Mars ("I Just Might"), Harry Styles
("Aperture"), BTS ("Swim"), and Olivia Rodrigo ("Drop Dead").

**Spotify, most-streamed 2026:** by monthly listeners, the order runs
Michael Jackson, Bad Bunny, Justin Bieber, Drake, Taylor Swift, BTS, Bruno
Mars, The Weeknd, Fuerza Regida. All-time streamed-song leader is still
The Weeknd's "Blinding Lights."

Source note: Wikipedia's Billboard Hot 100 2026 pages, Variety and
RouteNote's Spotify all-time coverage, thatericalper.com's monthly-updated
streaming numbers.

## The protocol

Per study, about 40 minutes. It produces one song entry, so **studies count
toward the 1,000.**

1. **Separate.** `tools\stems.ps1 "song.mp3"` splits it into drums, bass,
   vocals, other. Now you can hear one part at a time.
2. **Transcribe the bass first.** Bass tells you the harmony faster than the
   chords do. Write it out.
3. **Log the spec.** Create the song entry with `key`, `tempo`, `chords`,
   `instruments` filled in, and `tags: [study]`. This is the commit.
4. **Name the move.** One sentence: what does this song do that you want to
   steal? Not "it is good." Something like "the chorus lands on the minor iv
   and that is the whole ache."
5. **Fork it.** Same tempo, same changes, your melody. Ship it as a sketch.
6. **Vary, then choose.** Added 2026-09-13, after four Suno demos in a week
   made this the real loop. Sing the fork into a voice memo, placeholder words
   allowed (McCartney sang "scrambled eggs" over the Yesterday melody for
   months before the real words came). Upload it to Suno and take three or
   four variations, not one. Put them in Strummer DJ with a `suno:` link on
   the song file. Whichever you replay wins; that is the curation, and it is
   the only step in this protocol that costs nothing to repeat. Rewrite the
   winner's words. Then finish it, or leave it as a demo and move on. Both
   count.

Rule: never study without forking in the same session. A study you did not
fork is consumption wearing a lab coat.

Second rule, from the same week: the round is ten demos, then a listen. Below
ten, make the next one. At ten, pick the one to three that you keep replaying
and carry one lesson forward. Private demos count in full. Publishing is a
separate decision made later, not a condition of making.

## The queue

Ordered so that the earliest studies feed the templates you already have.

### Run 1 · Lennon and McCartney (feeds Template C)
1. **Something** (Harrison) · the chromatic descending bass line
2. **In My Life** · how a simple melody carries a modal turn
3. **Hey Jude** · the McCartney No. 1, and how long you can hold a coda
4. **Blackbird** · one guitar, one voice, complete arrangement
5. **Here, There and Everywhere** · deliberately plain, diatonic harmony, no key change (corrected 2026-08-10 after verified study, see no.075)

### Run 2 · The minor iv and the sad-major move (feeds Template C)
6. **The Lemon Twigs, My Golden Years** · your own top 10, 70s power pop
7. **Tennis, anything from Yours Conditionally** · jazzy retro chords
8. **Billie Eilish, BIRDS OF A FEATHER** · Finneas voicings under a plain top line

### Run 3 · Groove traditions (feeds Template D)
9. **Bob Marley, any** · one drop, and how much space a groove can hold.
   The exercise, added 2026-09-13: the kick and snare land together on beat
   three only, beats one and two are empty, and the guitar skank fills the
   offbeats. Fork it by taking a song you already have (2 afternoons or
   girl bossa) and moving its kick off beat one. If the song survives the
   space, the groove was carrying it. If it collapses, the melody was, and you
   have learned which of your own songs are which.
10. **Gipsy Kings, Ciento** · rumba, palmas, no drum kit
11. **Laufey, From The Start** · bossa comping, ii-V motion
12. **Vampire Weekend, any** · interlocking guitar lines

### Run 4 · Texture and loop (feeds Template B)
13. **Aphex Twin, Xtal** · your #1. What is actually in it? Separate it and see
14. **Aphex Twin, Avril 14th** · already Template A. Transcribe it properly
15. **NewJeans, any** · what 250 leaves OUT of a mix

### Run 5 · The craftsman
16. **Max Martin, three songs across three decades** · find the shared skeleton

### Run 6 · Constraint as a workflow, K-pop as a technique set (feeds Template H)
17. **Rebuild one Rick Rubin session on purpose** · no DAW for the writing pass, hum/hand-clap/tape a full idea before opening Ableton at all
18. **Illit, Magnetic** · transcribe just the kick/snare/hat pattern, then place your own bass and hats on the offbeat
19. **IVE, Hea** · same topline, new chords under the final chorus. Do this exact move to one of your own sketches
20. **A 250 track (see NewJeans catalog)** · count what's NOT in the mix. Subtraction as arrangement

## Why this is not stealing

You are learning grammar, not copying sentences. Chord progressions and tempos
are not copyrightable; specific melodies and recordings are. Study the
structure, write your own melody over it, and you are doing what every
songwriter in both tables above did. McCartney learned by playing other
people's records until he could hear how they worked.

Log studies as `tags: [study]` and keep them `draft: true` until they are your
own work.
