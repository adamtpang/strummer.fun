...
export default function Compare({ currentUser }: CompareProps) {
  ...
  function handleShare() {
    ...
    const url = `${window.location.origin}/vibe/share/compare/${idA}/${idB}`;
  ...
  return (
    <div className="ambient-bg min-h-screen bg-[#0a0908] overflow-hidden">
      <motion.div
...
function CompareCard({ vibe, side }: { vibe: VibeData; side: 'left' | 'right' }) {
  const gradient = vibe.vibe_gradient || 'linear-gradient(135deg, #1a1816, #211e1b, #121110)';
  ...
  return (
    <div>
      ...
      <div className="relative h-2 w-full rounded-full bg-white/10 overflow-hidden">
        <div
          className="absolute left-0 top-0 h-full rounded-full bg-gradient-to-r from-[#dc2626] to-[#7f1d1d] transition-all duration-700"
          style={{ width: `${value}%` }}
        />
      </div>
    </div>
